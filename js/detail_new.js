...
Promise.all([
  ...
  APP.loadJSON('fotocasa_data/agents.json').catch(() => ({})),
  APP.loadJSON('fotocasa_data/overrides.json').catch(() => ({}))
]).then(([data, agents, ovr]) => {
  AGENTS = agents || {};
  P = Object.assign({}, data.property_data, (ovr || {})[String(id)] || {});

  /* FIX 1: original listing URL (top-level source_url, fallback language_urls) */
  P.source_url = String(P.source_url || data.source_url || '').trim() ||
    (P.language_urls && P.language_urls.en_US
      ? 'https://www.fotocasa.es' + String(P.language_urls.en_US).trim()
      : '');

  /* Backslash-safe path normalization — NO regex, copy-paste proof */
  const BS = String.fromCharCode(92);   // the "\" character
  const dl = (P.downloaded_images || []).map(x => x.split(BS).join('/'));
...
/* Regex-free phone formatter — copy-paste proof */
const fmtPhone = n => {
  n = String(n || '').trim();
  return (n.indexOf('+34') === 0 && n.length === 12)
    ? '+34 ' + n.slice(3, 6) + ' ' + n.slice(6, 9) + ' ' + n.slice(9)
    : n;
};
...
function renderAll() {
  ...
  $('#title').textContent = `${APP.prettifyType(P.property_type)} in ${a.neighbourhood || a.district || a.city || (P.breadcrumb || []).slice(-1)[0] || ''}`;
  ...
  $('#facts').innerHTML = `
    ...
    ${fact('area', P.surface != null ? P.surface + ' m²' : null, 'Surface')}
  ...
  /* details table — missing values show "Not specified" */
  const row = (k, v) => `<div class="drow"><span class="k">${k}</span><span class="v">${v || '<span class="muted">Not specified</span>'}</span></div>`;
  $('#detailsTable').innerHTML = [
    row('Reference', P.reference), row('Created', (P.creation_date || '').slice(0, 10)),
    row('Heating', P.heating === 'YES' ? 'Yes' : (APP.prettifyEnum(P.heating) === '—' ? null : APP.prettifyEnum(P.heating))),
    row('Hot water', APP.prettifyEnum(P.hot_water) === '—' ? null : APP.prettifyEnum(P.hot_water)),
    row('Parking', APP.prettifyEnum(P.parking) === '—' ? null : APP.prettifyEnum(P.parking)),
    row('Elevator', P.has_elevator == null ? null : (P.has_elevator ? 'Yes' : 'No')),
    row('Furnished', P.is_furnished == null ? null : (P.is_furnished ? 'Yes' : 'No')),
    row('Age', APP.prettifyEnum(P.age) === '—' ? null : APP.prettifyEnum(P.age)),
    row('Energy', P.energy_rating ? `${P.energy_rating} (${P.energy_value ?? '–'})` : null),
    row('Emissions', P.emissions_rating ? `${P.emissions_rating} (${P.emissions_value ?? '–'})` : null),
    /* FIX 1 (part 2): link uses repaired P.source_url */
    row('Source', P.source_url
      ? `<a style="color:var(--teal)" target="_blank" rel="noopener" href="${P.source_url}">Original listing ↗</a>`
      : null),
  ].join('');
  ...
  if (agent.phone) { $('#callBtn').href = 'tel:' + agent.phone; $('#agentPhone').textContent = fmtPhone(agent.phone); }
  else { $('#callBtn').style.display = 'none'; $('#agentPhone').style.display = 'none'; }
  ...
  /* contact form */
  $('#msgBox').value = `Please contact me regarding ${APP.prettifyType(P.property_type)} in ${(P.breadcrumb || []).slice(-1)[0] || a.city || ''} (ref ${P.reference || id}).`;
  ...
  APP.loadJSON('fotocasa_data/index.json').then(idx => {
    const town = (P.breadcrumb || []).slice(-1)[0];
    const sim = idx.filter(x => x.id !== P.property_id && (x.province === a.province || x.city === a.city || x.city === town)).slice(0, 3);
...
function fact(icon, v, l) {
  return `<div class="fact">${APP.icon(icon)}<div><div class="v">${v ?? 'N/A'}</div><div class="l">${l}</div></div></div>`;
}

/* lightbox + street-view modal */
let lbI = 0;
